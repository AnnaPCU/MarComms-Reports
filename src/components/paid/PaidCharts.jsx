...
export function PaidCharts({ campaigns = [], currency = 'EUR', lang = 'es' }) {
  ...
  return (
    <div className="mb-5 grid grid-cols-1 gap-3 lg:grid-cols-2">
      <ChartCard title={L.chInvTitle} subtitle={L.chInvSub(currency)}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={invData} margin={{ left: 0, right: 8, top: 6, bottom: 4 }}>
            ...
            <Tooltip
              {...CHART_TOOLTIP}
              formatter={(v, n) => (n === L.sCost ? `${numEs(v)} ${currency}` : numEs(v))}
              cursor={{ fill: 'rgba(62,178,237,.06)' }}
            />
            <Legend wrapperStyle={{ fontSize: 10 }} />
            <Bar yAxisId="left" dataKey="cost" name={L.sCost} fill={CU.dblue} radius={[4, 4, 0, 0]} maxBarSize={34} />
            <Line yAxisId="right" dataKey="clicks" name={L.sClicks} stroke={CU.cyan} strokeWidth={2.5} dot={{ r: 3, fill: CU.cyan }} />
          </ComposedChart>
        </ResponsiveContainer>
      </ChartCard>

      <ChartCard title={L.chImpTitle} subtitle={L.chImpSub}>
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Tooltip {...CHART_TOOLTIP} formatter={(v) => `${numEs(v)} ${L.impressionsWord}`} />
