...
export function WebsiteAnnualReview({ account, lang = 'es' }) {
  ...
  return (
    <div className="animate-fade-in">
      <SectionHeader title={t.yTitle} note={t.yNote(accName)} />
      <div className="mb-4 rounded-cu border border-cu-border border-l-4 border-l-cu-cyan bg-white px-4 py-3 text-[12px] text-cu-dgrey shadow-cu">
        <strong className="text-cu-dblue">{t.yQuarters(activeLabels)}</strong>
      </div>

      <SectionHeader title={t.yKpiSection} note={accName} />
      <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <KpiCard label={t.kVisitors} value={num(tt.visitors)} delta={{ dir: 'flat', label: t.sessionsLabel(num(tt.total)) }} />
        <KpiCard label={t.kPageviews} value={num(tt.pageviews)} />
        <KpiCard
          label={t.kConversions}
        ...
        {tt.hasSeo ? (
          <KpiCard
            ...
            delta={{ dir: 'flat', label: t.seoImpLabel(num(tt.seoImp)) }}
            footnote={t.seoPosFoot(pos(tt.avgPos))}
          />
        ) : (
          <KpiCard label={t.kSeoClicks} value="—" footnote={t.noSeoFoot} />
        )}
      </div>

      <SectionHeader title={t.yEvoSection} note={t.yEvoNote} />
      <div className="mb-5 grid gap-3 lg:grid-cols-2">
        <ChartCard title={t.yChVisTitle} subtitle="GA4">
          <ResponsiveContainer>
            <ComposedChart data={series} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              ...
              <Bar dataKey="visitors" name={t.dkVisitors} fill={CU.cyan} radius={[4, 4, 0, 0]} maxBarSize={30} />
              <Line dataKey="conversions" name={t.dkConversions} stroke={CU.dblue} strokeWidth={2} dot={{ r: 3.5, fill: CU.dblue, strokeWidth: 2, stroke: '#fff' }} />
            </ComposedChart>
          </ResponsiveContainer>
        </ChartCard>
        {tt.hasSeo && (
        <ChartCard title={t.yChSeoTitle} subtitle="Search Console">
          <ResponsiveContainer>
            <BarChart data={series} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="0" stroke={CU.border2} vertical={false} />
              <XAxis dataKey="name" tick={{ fontSize: 11, fill: CU.grey }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 10, fill: CU.grey }} axisLine={false} tickLine={false} width={44} />
              <Tooltip {...CHART_TOOLTIP} formatter={(v) => [num(v), t.kSeoClicks]} />
      ...
      <SectionHeader title={t.yDetailSection} />
        ...
        <table className="w-full min-w-[720px] border-collapse">
          ...
          <tbody>
            ...
            <tr className="bg-cu-bg/60">
              <td className={`${tdCls} font-bold text-cu-dblue`}>{t.totalWord}</td>
            ...
            <tbody>
              {year.topPages.map((p) => (
                <tr key={p.key} className="border-b border-cu-border2 last:border-b-0">
                  ...
                  <td className={`${tdCls} whitespace-nowrap text-right font-medium text-cu-dblue`}>{num(p.value)} {t.viewsWord}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody>
              {year.topKeywords.map((k) => (
                <tr key={k.key} className="border-b border-cu-border2 last:border-b-0">
                  ...
                  <td className={`${tdCls} whitespace-nowrap text-right font-medium text-cu-dblue`}>{num(k.value)} {t.clicksWord}</td>
                </tr>
              ))}
            </tbody>
          </table>
